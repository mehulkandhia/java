...
public class OrderController {
	...
	@RequestMapping(value="/payment/{productid}/addition")
	public String paymentprocess(@PathVariable("productid")int productid,@RequestParam("quantity")int quantity,@RequestParam("pmode")String pmode,HttpSession session,Model m)
	{
		Cart cart1=new Cart();
	...
	@RequestMapping(value="/confirm/{productid}/addition")
	public String confirm(@PathVariable("productid")int productid,@RequestParam("quantity")int quantity,Model m, String username,HttpSession session)
	{
		Cart cart1=new Cart();
		product prod=production.getproduct(productid);
		System.out.println("ProductId :"+prod.getProductid()+"Quantity of product :"+prod.getStock()+"Quantity in cart :"+cart1.getQuantity());
